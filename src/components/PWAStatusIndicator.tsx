...
const PWAStatusIndicator: React.FC = () => {
  ...
  React.useEffect(() => {
    // ?¤íƒ ?œì–¼ë¡?ëª¨ë“œ?ì„œ ?Œë¦¼ ê¶Œí•œ???†ìœ¼ë©??„ë¡¬?„íŠ¸ ?œì‹œ
  ...
  const handleEnableNotifications = async () => {
    try {
      ...
      if (permission === 'granted') {
        // ?Œë¦¼ ?ˆìš© ???˜ì˜ ?Œë¦¼ ?œì‹œ
        new Notification('ë°”ì´ì¹??¬ê?ë³µì??¼í„°', {
          body: '?Œë¦¼???œì„±?”ë˜?ˆìŠµ?ˆë‹¤! ì¤‘ìš”???ë‹´ ?•ë³´ë¥??“ì¹˜ì§€ ë§ˆì„¸??',
    ...
    <>
      ...
      >
        <div className="flex items-center gap-2 bg-white/90 dark:bg-secondary-800/90 backdrop-blur-sm px-3 py-2 rounded-full border border-secondary-200 dark:border-secondary-700 shadow-lg">
          ...
          <span className="text-sm font-medium text-secondary-700 dark:text-secondary-300">
            {isOnline ? (
              connectionType !== 'unknown' ? connectionType.toUpperCase() : '?¨ë¼??
            ) : (
              '?¤í”„?¼ì¸'
            )}
          </span>
          ...
          {isStandalone && (
            <div className="w-2 h-2 bg-primary-500 rounded-full animate-pulse" title="??ëª¨ë“œ" />
          )}
        </div>
      </motion.div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-primary-700 dark:text-primary-300 mb-3">
                    ?ˆë¡œ??ê¸°ëŠ¥ê³?ê°œì„ ?¬í•­??ì¤€ë¹„ë˜?ˆìŠµ?ˆë‹¤.
                  </p>
      ...
      {process.env.NODE_ENV === 'development' && (
        <motion.div
          ...
          transition={{ delay: 2 }}
        >
          PWA: {isStandalone ? '??ëª¨ë“œ' : 'ë¸Œë¼?°ì? ëª¨ë“œ'} | {connectionType}
        </motion.div>
      )}
    </>
  );
